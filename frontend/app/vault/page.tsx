  ...
  return (
    <main className="mx-auto w-full max-w-[1400px] flex-1 px-5 py-8">
      ...
      {payload && !payload.index_ready && (
        <div className="mt-4 rounded-md border border-ochre/30 bg-ochre-tint px-4 py-3.5">
          <p className="flex items-center gap-2 text-[13px] font-medium text-ochre-ink">
            <TriangleAlert className="h-4 w-4" />
            The retrieval index is empty
          </p>
          <p className="mt-1.5 text-[12px] leading-relaxed text-ochre-ink">
      ...
      {error && (
        <div className="mt-4 rounded-md border border-rust/30 bg-rust-tint px-4 py-3.5">
      ...
      <Card className="mt-6">
        <CardHead
          ...
          right={
            <div className="flex items-center gap-2 rounded-md border border-line bg-surface-2 px-2.5 focus-within:border-forest">
              <Search className="h-3.5 w-3.5 shrink-0 text-meta" />
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Search titles and filenames"
                aria-label="Search the regulatory index"
                className="w-52 bg-transparent py-1.5 text-[12px] text-ink outline-none placeholder:text-meta"
              />
            </div>
          }
        />
        ...
        {loading ? (
          <p className="px-5 py-10 text-center text-[13px] text-meta">
            Reading the corpus…
          </p>
        ) : rows.length === 0 ? (
          <Empty
                ...
                {rows.map((d) => (
                  ...
                  >
                    <td className="px-5 py-3.5">
                      ...
                      <p className="figure mt-0.5 break-all text-[11px] text-meta">
                        {d.file}
                      </p>
                    </td>
                    <td className="px-5 py-3.5">
                      <Pill tone={d.meta.issuer === "Unattributed" ? "neutral" : "sage"}>
                        {d.meta.issuer}
                      </Pill>
                    ...
                    <td className="figure px-5 py-3.5 text-right text-[13px] text-meta">
                      {mb(d.bytes)}
                    </td>
                    <td className="px-5 py-3.5">
                      {d.indexed ? (
                    ...
                    <td className="px-5 py-3.5 text-right">
                      {d.bytes !== null && (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 rounded-md border border-line px-2.5 py-1 text-[12px] text-ink transition hover:border-line-strong"
                        >
        ...
        <div className="border-t border-line px-5 py-3.5">
          <p className="flex items-start gap-1.5 text-[11px] leading-relaxed text-meta">

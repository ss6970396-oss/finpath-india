...
- `frontend/AGENTS.md` imports `frontend/AGENTS.md`, which is **generated and re-added by `next dev`**. Do not delete it from a diff; commit it with your work to keep the tree clean. It requires reading `node_modules/next/dist/docs/` before writing Next.js code, since Next 16 has breaking changes (async request APIs, Turbopack default, `next lint` removed, `middleware` → `proxy`).
